function varargout = fun_polynomial_2D(f, A, B, u, v, l)
%FUN_POLYNOMIAL Evaluate f(I \otimes A - B^T \otimes I) vec(u*v') in matrix form using polynomial Krylov subspaces

polesA = inf(1, l); 
polesB = polesA;

% Construct the rational Krylov subspaces
[VA, ~, ~] = rat_krylov(A,   u, polesA);
[VB, ~, ~] = rat_krylov(B.', v, polesB);

VA = VA(:, 1:end-1);
VB = VB(:, 1:end-1);

Al = VA' * A * VA;
Bl = VB' * B.' * VB;

% Make them symmetric
Al = (Al + Al')/2;
Bl = (Bl + Bl')/2;

% Project the RHS
ul = VA' * u;
vl = VB' * v;

% Evaluate the small function
Y = fun_diag(f, Al, Bl.', ul, vl);

if nargout >= 3
    varargout{1} = VA;
    varargout{2} = Y;
    varargout{3} = VB;
elseif nargout == 2
    varargout{1} = VA * Y;
    varargout{2} = VB;
else
    error('Unsupported number of output arguments');
end

end

